...
vertex_IDs_generator<-function(df_anpoll_nodeID_aggregated,output_path,analyte=T,
      polluter=F,graph_subset=T,rank_subgraph=2){
  #########handle the global variable issues###############
  graph_order = 0
  ########################################################
...
########################################################################################################
## Writing a function to extract a list of projected node IDs in the base river network for each polluter node
projected_nodeIDs_list_generator_new_polluter<-function(file_path,df_polluter_new, 
  projected_threshold_dist_km){
  #########handle the global variable issues###############
  df_node_latlong = 0
  df_anpoll_processed = 0
  df_analyte_nodeID_aggregated = 0
...
flow_dist_polluter_projected_cal<-function(file_path){
  #########handle the global variable issues###############
  projected_nodeIDs_list = 0
  igraph_river_whole = 0
  total_edgelist_character_modified = 0
  stream_path_dist_vec = 0
  ########################################################
...
anpoll_network_creator <- function(output_path){
  #########handle the global variable issues###############
  shortest_path_anpoll_edgelist = 0
  df_polluter_processed = 0
  df_anpoll_processed = 0
  df_analyte_nodeID_aggregated = 0
  df_polluter_nodeID_aggregated = 0
...
wrapper_flow_dist_cal_chunk_parallelized <- function(df_polluter,anpoll_edgelist, 
  shortest_path_anpoll_edgelist, total_edgelist_character_modified, from_indicator, 
  to_indicator,chunk_list,n_chunk, file_path){
  #########handle the global variable issues###############
  stream_path_dist_vec = 0
  df_projected_nodeIDs = 0
  igraph_river_whole = 0
  projected_nodeIDs_vec = 0
  polluter_vertex_ids = 0
  index = 0
...
polluter_test_dist_time <- function(df_polluter, polluter_lon, polluter_lat, polluter_projected_dist_km, 
    upstream_threshold_dist_km, downstream_threshold_lower_dist_km, 
    downstream_threshold_upper_dist_km, date_start, date_end, spill_date, file_path){
  #########handle the global variable issues###############
  anpoll_edgelist = 0
  flow_dist_to_list = 0
  projected_nodeIDs_list = 0
  flow_dist_polluter_projected_list = 0
  df_node_latlong_anpoll = 0
  listID_nodeID_matrix = 0
  list_analyte_time = 0
  ########################################################
  ...
  if((from_n_before_spill > 0) & (from_n_after_spill > 0)){
    ...
    if((from_n_before_spill > 1) & (from_n_after_spill > 1)){
      ...
      if(inherits(try(stats::t.test(x = from_obs_before_spill, y = from_obs_after_spill,alternative = "less")$p.value)
               ,"try-error")){
      ...
      if(inherits(try(stats::wilcox.test(x = from_obs_before_spill, y = from_obs_after_spill,alternative = "less", exact=F)$p.value),
                  "try-error")){
  ...
  if((to_n_before_spill > 0) & (to_n_after_spill > 0)){
    ...
    if((to_n_before_spill > 1) & (to_n_after_spill > 1)){
      if(inherits(try(stats::t.test(x = to_obs_before_spill,y = to_obs_after_spill,alternative = "less")$p.value),
                  "try-error")){
      ...
      #p_value_wilcox.test_2sided<-wilcox.test(x = from_obs_total,y = to_obs_total)$p.value
      if(inherits(try(stats::wilcox.test(x = to_obs_before_spill,y = to_obs_after_spill,alternative = "less")$p.value),
                  "try-error")){
  ...
  if((from_n_total > 0) & (to_n_after_spill > 0)){
    ...
    if((from_n_total > 1) & (to_n_after_spill > 1)){
      if(inherits(try(stats::t.test(x = from_obs_total, y = to_obs_after_spill,alternative = "less")$p.value,
                   silent = T),"try-error")){
      ...
      #p_value_wilcox.test_2sided<-wilcox.test(x = from_obs_total,y = to_obs_total)$p.value
      if(inherits(try(stats::wilcox.test(x = from_obs_total,
...
## Defining a wrapper function to calculate flow distance of all polluter nodes in parallel
wrapper_flow_dist_cal <- function(index, df_polluter,anpoll_edgelist, 
                                  shortest_path_anpoll_edgelist, 
                                  total_edgelist_character_modified, 
                                  from_indicator, to_indicator, flow_type, file_path){
  #########handle the global variable issues###############
  stream_path_dist_vec = 0
  ########################################################
...
wrapper_polluter_test_update <- function(index, n_chunks, df_polluter_to_append, file_path, j, 
  df_threshold_dist_km){
  #########handle the global variable issues###############
  df_polluter_processed = 0
  df_polluter_processed_to_append = 0
  df_anpoll_processed = 0
  df_analyte_nodeID_aggregated = 0
  igraph_river_whole = 0
  projected_nodeIDs_vec = 0
