...
########################################################################################################
# helper function to return and assign multiple values like in python
...
two_step_anpoll_mapper<-function(df_anpoll_preprocessed, output_path, df_node_latlong=NULL, total_edgelist_character=NULL, stream_list=NULL){
  ...
  mid_stream_node_creator<-function(flag_mid, total_edgelist_character_modified, stream_list_modified, df_anpoll_preprocessed){
    flag_mid<-flag_mid+1
    ...
    return (list(flag_mid, total_edgelist_character_modified, stream_list_modified, df_anpoll_preprocessed))
  }
  ###################################################
  ## Defining a function to map to node end of the stream and just update df_anpoll_preprocessed in the global environment
  end_stream_node_mapper<-function(df_anpoll_preprocessed){
    ## Appending the "df_anpoll_preprocessed" with node_ID, lon_mapped and lat_mapped
  ...
  ## Two step mapping loop for each row in df_anpoll_preprocessed; 1st step: Get top ten nearest node IDs, 2nd step: Create new nodes and streams by mapping to the river streams corresponding to top ten nearest node IDs
    ...
    ## foreach(i = 1:nrow(df_anpoll_preprocessed), .packages="geosphere") %dopar% {
    for(i in 1:nrow(df_anpoll_preprocessed)) {
    dist_vec_step1<-geosphere::distm(x = as.matrix(df_anpoll_preprocessed[i,c("lon","lat")]),y = as.matrix(df_node_latlong_modified[,c("lon","lat")]))
    ...
    for (j in 1:length(stream_sub_list)){
      ...
      dist_to_stream[j,]<-geosphere::dist2Line(p = as.matrix(df_anpoll_preprocessed[i,c("lon","lat")]),line = stream_sub_list[[j]][,c(1,2)])
    }
    ...
    dist_to_points<-as.vector(geosphere::distm(x = as.matrix(df_anpoll_preprocessed[i,c("lon","lat")]), y = as.matrix(stream_sub_df[stream_selected_row_IDs,c(1,2)])))
    if(order(dist_to_points)[1]%in%c(1,length(dist_to_points))){
      if(order(dist_to_points)[1]==1){
        dist_12<-as.numeric(geosphere::distm(x = as.matrix(stream_sub_df[stream_selected_row_IDs[1],c(1,2)]), y = as.matrix(stream_sub_df[stream_selected_row_IDs[2],c(1,2)])))
        ...
        if(!is.nan(angle_CPP_1_2)){
          if((angle_CPP_1_2>=90)|(coincide_condition)){
            df_anpoll_preprocessed <- end_stream_node_mapper( df_anpoll_preprocessed )
          }else{
            list[flag_mid, total_edgelist_character_modified, stream_list_modified, df_anpoll_preprocessed] <-  mid_stream_node_creator(flag_mid, total_edgelist_character_modified, 
                                                                                                                                        stream_list_modified, df_anpoll_preprocessed)
          }
        }else{
        ...
        dist_end_12<-as.numeric(geosphere::distm(x = as.matrix(stream_sub_df[stream_selected_row_IDs[length(stream_selected_row_IDs)-1],c(1,2)]), y = as.matrix(stream_sub_df[stream_selected_row_IDs[length(stream_selected_row_IDs)],c(1,2)])))
...
two_step_anpoll_mapper_update<-function(df_anpoll_preprocessed, output_path, df_node_latlong=NULL, total_edgelist_character=NULL, stream_list=NULL, df_node_latlong_modified=NULL){ 
  file_path <- output_path
  ## Loading the dataframe "df_node_latlong" from common files
  load(file = paste0(output_path,"common_files/df_node_latlong.RData"))
  ## Loading the "total_edgelist_character" from common files
  ...
  dist_mapped <- cbind(dist_mapped, dist_mapped_to_append)
  save(dist_mapped,file=paste0(output_path,"anpoll_files/dist_mapped.RData"))
...
two_step_anpoll_mapper_update_polluter<-function(df_anpoll_preprocessed, output_path, df_node_latlong=NULL, total_edgelist_character=NULL, stream_list=NULL){
  file_path <- output_path
  ## Loading the dataframe "df_node_latlong" from common files
  load(file = paste0(output_path,"common_files/df_node_latlong.RData"))
  ## Loading the "total_edgelist_character" from common files
      ...
      if(order(dist_to_points)[1]==1){
        ...
        if(!is.nan(angle_CPP_1_2)){
          if((angle_CPP_1_2>=90)|(coincide_condition)){
            ...
            list[flag_mid, total_edgelist_charachter_modified, stream_list_modified, df_anpoll_preprocessed] <- mid_stream_node_creator(flag_mid, total_edgelist_character_modified, 
                                                                                                                                  stream_list_modified, df_anpoll_preprocessed)
          }
        }else{
          if(coincide_condition){
  ...
  df_anpoll_preprocessed$anpoll_indicator <- "polluter"
  df_anpoll_preprocessed$conc <- NA
  ## Append df_anpoll_processed with df_anpoll_preprocessed
  df_anpoll_processed<-rbind(df_anpoll_processed,df_anpoll_preprocessed)
...
stream_path_dist_cal<-function(output_path, total_edgelist_character_modified=NULL, stream_list_modified=NULL){
  ...
  for(i in 1:nrow(total_edgelist_character_modified)){
    for (j in 1:(nrow(stream_list_modified[[i]])-1)){
      stream_path_dist_vec[i]<-stream_path_dist_vec[i]+as.numeric(geosphere::distm(x = stream_list_modified[[i]][j,],y = stream_list_modified[[i]][j+1,]))
    }
  }
...
river_net_subset<-function(output_path, total_edgelist_character_modified=NULL){
  ## Loading the "total_edgelist_character_modified"
  load(file = paste0(output_path,"common_files_modified/total_edgelist_character_modified.RData"))
  ## Creating the igraph object from "total_edgelist_character_modified"
  igraph_river_whole<-igraph::graph_from_edgelist(total_edgelist_character_modified[,c(1,2)], directed = TRUE)
  ...
  connected_clusters<-igraph::clusters(igraph_river_whole)
  ...
  igraph_river_decomposed_list <- igraph::decompose.graph(igraph_river_whole)
...
vertex_IDs_generator<-function(df_anpoll_nodeID_aggregated,output_path,analyte=T,polluter=F,graph_subset=T,rank_subgraph=2, igraph_river_whole=NULL, graph_order=NULL, igraph_river_decomposed_list=NULL){
  ...
  if(graph_subset){
    if(analyte){
      ...
      igraph_river_decomposed_list[[graph_order[rank_subgraph]]]<-igraph::set_vertex_attr(graph = igraph_river_decomposed_list[[graph_order[rank_subgraph]]],name = "analyte",index = igraph::V(igraph_river_decomposed_list[[graph_order[rank_subgraph]]]),value = igraph::V(igraph_river_decomposed_list[[graph_order[rank_subgraph]]])$name%in%df_anpoll_nodeID_aggregated$nodeID)
      ## extracting vertex ids corresponding to observation locations
      analyte_vertex_ids<-which(igraph::vertex_attr(graph = igraph_river_decomposed_list[[graph_order[rank_subgraph]]])$`analyte`==TRUE)
      ...
      analyte_nodeIDs_rank_subgraph<-igraph::V(igraph_river_decomposed_list[[graph_order[rank_subgraph]]])$name[analyte_vertex_ids]
      ...
      ## setting vertex attributes as observation being true/false
      igraph_river_decomposed_list[[graph_order[rank_subgraph]]]<-igraph::set_vertex_attr(graph = igraph_river_decomposed_list[[graph_order[rank_subgraph]]],name = "polluter",index = igraph::V(igraph_river_decomposed_list[[graph_order[rank_subgraph]]]),value = igraph::V(igraph_river_decomposed_list[[graph_order[rank_subgraph]]])$name%in%df_anpoll_nodeID_aggregated$nodeID)
      ## extracting vertex ids corresponding to observation locations
      polluter_vertex_ids<-which(igraph::vertex_attr(graph = igraph_river_decomposed_list[[graph_order[rank_subgraph]]])$`polluter`==TRUE)
      ...
      polluter_nodeIDs_rank_subgraph<-igraph::V(igraph_river_decomposed_list[[graph_order[rank_subgraph]]])$name[polluter_vertex_ids]
      ## Appending the "df_anpoll_nodeID_aggregated" with a column rank_subgraph_indicator that is TRUE or FALSE depending on whether the corresponding nodes are in rank_subgraph
    ...
    if(analyte){
      ## setting vertex attributes as observation being true/false
      igraph_river_whole<-igraph::set_vertex_attr(graph = igraph_river_whole,name = "analyte",index = igraph::V(igraph_river_whole),value = igraph::V(igraph_river_whole)$name%in%df_anpoll_nodeID_aggregated$nodeID)
      ## extracting vertex ids corresponding to observation locations
      analyte_vertex_ids<-which(igraph::vertex_attr(graph = igraph_river_whole)$`analyte`==TRUE)
      ## saving the vertex ids in rank subgraph corresponding to analyte observations unique sites
      save(analyte_vertex_ids,file = paste0(output_path,"analyte_files/analyte_vertex_ids.RData"))
      anpoll_vertex_ids<-analyte_vertex_ids
    }else if(polluter){
      ## setting vertex attributes as observation being true/false
      igraph_river_whole<-igraph::set_vertex_attr(graph = igraph_river_whole,name = "polluter",index = igraph::V(igraph_river_whole),value = igraph::V(igraph_river_whole)$name%in%df_anpoll_nodeID_aggregated$nodeID)
      ## extracting vertex ids corresponding to observation locations
      polluter_vertex_ids<-which(igraph::vertex_attr(graph = igraph_river_whole)$`polluter`==TRUE)
      ## saving the vertex ids in rank subgraph corresponding to polluter observations unique sites
      save(polluter_vertex_ids,file = paste0(output_path,"polluter_files/polluter_vertex_ids.RData"))
      anpoll_vertex_ids<-polluter_vertex_ids
    }
...
projected_nodeIDs_list_generator<-function(file_path,projected_threshold_dist_km,
                                            df_anpoll_processed=NULL, df_polluter_processed=NULL,
                                            df_node_latlong=NULL, total_edgelist_character_modified=NULL,
                                            total_edgelist_character=NULL, igraph_river_whole=NULL){
  ...
  for(i in 1:nrow(df_polluter_processed)){
    ...
    dist_vec<-geosphere::distm(x = as.matrix(df_polluter_processed[i,c("lon","lat")]),y = as.matrix(df_node_latlong[,c("lon","lat")]))
    ...
    vertex_IDs_site_i <- which(igraph::V(igraph_river_whole)$name %in% node_IDs_site_i)
    #vertex_IDs_site_i<-vertex_IDs_all
    ## Getting the shortest path from polluter_i to all the subsetted nodes
    #ptm<-proc.time()
    short_path <- igraph::shortest_paths(graph = igraph_river_whole, from = which(igraph::V(igraph_river_whole)$name %in% df_polluter_processed$nodeID[i]), to = vertex_IDs_site_i,mode = "out",output = "vpath")
    #print(proc.time()-ptm)
...
projected_nodeIDs_list_generator_new_polluter<-function(file_path,df_polluter_new, projected_threshold_dist_km,
                                                        df_anpoll_processed=NULL, df_polluter_processed=NULL,
                                                        df_node_latlong=NULL,total_edgelist_character=NULL,
                                                        total_edgelist_character_modified=NULL,igraph_river_whole=NULL,
                                                        projected_nodeIDs_list=NULL){
  ## Loading the dataframe df_polluter_nodeID_aggregated
  load(file = paste0(file_path,"anpoll_files/df_anpoll_processed.RData"))
  ## Loading the dataframe df_polluter_processed
  load(file = paste0(file_path,"polluter_files/df_polluter_processed.RData"))
  ...
  for(i in 1:nrow(df_polluter_new)){
    ...
    ## Getting the node ids of top 1000 nearest nodes
    dist_vec<-geosphere::distm(x = as.matrix(df_polluter_new[i,c("lon","lat")]),y = as.matrix(df_node_latlong[,c("lon","lat")]))
    ## Getting the node IDs which lie within the projected_distance_threshold_km 
    node_IDs_site_i <- df_node_latlong$nodeID[which(dist_vec<=(projected_threshold_dist_km*1000))]
    ...
    ## Getting the shortest path from polluter_i to all the subsetted nodes
    #ptm<-proc.time()
    short_path <- igraph::shortest_paths(graph = igraph_river_whole, from = which(igraph::V(igraph_river_whole)$name %in% df_polluter_new$nodeID[i]), to = vertex_IDs_site_i,mode = "out",output = "vpath")
    #print(proc.time()-ptm)
    ## Trimming the paths with length 0 and storing all paths (among nodeIDs) in a list
    short_path_list<-lapply(X = short_path$vpath,FUN = function(x){
      if(length(igraph::as_ids(x))>0){
...
flow_dist_polluter_projected_cal<-function(file_path, igraph_river_whole=NULL,total_edgelist_character_modified=NULL,
                                            stream_path_dist_vec=NULL, projected_nodeIDs_list=NULL){
  ...
  ## Loading the projected node IDs list
  load(file=paste0(file_path,"polluter_files/projected_nodeIDs_list.RData"))
  Rcpp::sourceCpp( paste0(file_path, "code/flow_dist_cal_cpp.cpp") , env=environment())
  ...
  flow_dist_cal<-function(short_path_vec, test_node_ID, current_node_ID, file_path){
    ...
    #   flow_dist_m<-flow_dist_m+as.numeric(stream_path_dist_vec[which((total_edgelist_character_modified[,1]==flow_path_nodeIDs_mat[k,1])&(total_edgelist_character_modified[,2]==flow_path_nodeIDs_mat[k,2]))])
    # }
    
    flow_dist_m  <- getDistance(flow_path_nodeIDs = flow_path_nodeIDs_mat,
  ...
  for (i in 1:length(projected_nodeIDs_list)){
    ...
    if(length(projected_nodeIDs_list[[i]])>0){
      ## Extracting vertex ids corresponding to these subsetted nodeIDs
      vertex_IDs_projected_i<-which(igraph::V(igraph_river_whole)$name%in%projected_nodeIDs_list[[i]])
      #vertex_IDs_site_i<-vertex_IDs_all
      ## Getting the shortest path from polluter_i to all the subsetted nodes
      #ptm<-proc.time()
      short_path<-igraph::shortest_paths(graph = igraph_river_whole,from=which(igraph::V(igraph_river_whole)$name%in%current_node_ID), to= vertex_IDs_projected_i,mode = "out",output = "vpath")
      #print(proc.time()-ptm)
      ## Trimming the paths with length 0 and storing all paths (among nodeIDs) in a list
      short_path_list<-lapply(X = short_path$vpath,FUN = function(x){
        if(length(igraph::as_ids(x))>0){
...
shortest_path_edgelist_creator<-function(graph_subset=T,rank_subgraph=2,file_path, analyte_vertex_ids=NULL, polluter_vertex_ids=NULL,
                                          projected_nodeIDs_vec=NULL, igraph_river_whole=NULL){
  library(igraph)
  ...
  for(i in 1:length(analyte_polluter_projected_vertex_ids)){
    ptm<-proc.time()
    short_path<-igraph::shortest_paths(graph = igraph_river_whole,from=analyte_polluter_projected_vertex_ids[i], to= analyte_polluter_projected_vertex_ids[-i],mode = "out",output = "vpath")
    ## Trimming the paths with length 0 and storing all paths (among nodeIDs) in a list
...
shortest_path_edgelist_creator_parallelized<-function(segment_index,file_path, analyte_vertex_ids=NULL, polluter_vertex_ids=NULL,
                                          igraph_river_whole=NULL, projected_nodeIDs_vec=NULL){
  library(igraph)
  ## Loading the analyte and polluter vertex ids
  load(file = paste0(file_path,"analyte_files/analyte_vertex_ids.RData"))
  load(file = paste0(file_path,"polluter_files/polluter_vertex_ids.RData"))
  ...
  if(segment_index!=length(segment_start_indices)){
    for(i in 1:segment_length){
      short_path<-igraph::shortest_paths(graph = igraph_river_whole,from=analyte_polluter_projected_vertex_ids[segment_start_indices[segment_index]+(i-1)], to= analyte_polluter_projected_vertex_ids[-(segment_start_indices[segment_index]+(i-1))],mode = "out",output = "vpath")
      ## Trimming the paths with length 0 and storing all paths (among nodeIDs) in a list
      edge_list_index[[i]]<-lapply(X = short_path$vpath,FUN = function(x){
        if(length(igraph::as_ids(x))>0){
          y<-igraph::as_ids(x)
    ...
    if((length(analyte_polluter_projected_vertex_ids)%%segment_length)>0){
      for(i in 1:(length(analyte_polluter_projected_vertex_ids)%%segment_length)){
        short_path<-igraph::shortest_paths(graph = igraph_obj,from=analyte_polluter_projected_vertex_ids[segment_start_indices[segment_index]+(i-1)], to= analyte_polluter_projected_vertex_ids[-(segment_start_indices[segment_index]+(i-1))],mode = "out",output = "vpath")
        ## Trimming the paths with length 0 and storing all paths (among nodeIDs) in a list
        edge_list_index[[i]]<-lapply(X = short_path$vpath,FUN = function(x){
          if(length(igraph::as_ids(x))>0){
            y<-igraph::as_ids(x)
...
## Writing a function (used later in foreach) to create shortest path edgelist and creating the network for analyte-polluter. Inputs are  1) analyte and polluter nodeID aggregated dataframes "df_analyte_nodeID_aggregated" and "df_polluter_nodeID_aggregated" 2) directory path where output files should be stored at. Output shortest path edgelist
shortest_path_edgelist_creator_parallelized_2<-function(index, n_chunks,file_path, analyte_vertex_ids=NULL, polluter_vertex_ids=NULL,
                                          igraph_river_whole=NULL, projected_nodeIDs_vec=NULL){
  library(igraph)
  ## Loading the analyte and polluter vertex ids
  load(file = paste0(file_path,"analyte_files/analyte_vertex_ids.RData"))
  ...
  for(i in 1:length(current_indices)){
    #ptm<-proc.time()
    short_path<-igraph::shortest_paths(graph = igraph_river_whole,from=analyte_polluter_projected_vertex_ids[current_indices[i]], to= analyte_polluter_projected_vertex_ids[-current_indices[i]],mode = "out",output = "vpath")
    ## Trimming the paths with length 0 and storing all paths (among nodeIDs) in a list
    edge_list_index[[i]]<-lapply(X = short_path$vpath,FUN = function(x){
      if(length(igraph::as_ids(x))>0){
        y<-igraph::as_ids(x)
...
anpoll_network_creator <- function(output_path, shortest_path_anpoll_edgelist=NULL, df_analyte_nodeID_aggregated=NULL, df_polluter_nodeID_aggregated=NULL){
...
# wrapper_flow_dist_cal <- function(index,df_polluter,anpoll_edgelist, shortest_path_anpoll_edgelist, total_edgelist_character_modified, from_indicator, to_indicator, file_path){
#   ## Defining a function to calculate between a test node and a current node. The current nodes are usually polluter nodes.
...
wrapper_flow_dist_cal_chunk_parallelized <- function(df_polluter,anpoll_edgelist, shortest_path_anpoll_edgelist, total_edgelist_character_modified, from_indicator, to_indicator,chunk_list,n_chunk, file_path, df_projected_nodeIDs=NULL, stream_path_dist_vec=NULL){
  ## Defining a function to calculate between a test node and a current node. The current nodes are usually polluter nodes.
  flow_dist_cal<-function(anpoll_edgelist, shortest_path_anpoll_edgelist, total_edgelist_character_modified, test_node_ID, current_node_ID, from_indicator, to_indicator, file_path){
    ## Getting the edge row ID
    if(from_indicator){
...
from_nodeIDs_crawler<-function(nodeID,upstream_threshold_dist_km,file_path, total_edgelist_character_modified=NULL, anpoll_edgelist=NULL, flow_dist_from_list=NULL){
...
## Writing a function to do inference that is testing the significance
## of polluter for given polluter coordinates, threshold flow distance
## and time interval, to collect all observations upstream and
## downstream within the specified threshold flow distance and time
## and do a two sample two sided t test and Wilcoxon rank sum
## test. Inputs are 1) latlong of polluter 2) threshold flow distance
## and 3) time interval. Outputs are mean, median and length of
## observations upstream and downstream within the specified threshold
## flow distance and year interval and the corresponding p values
polluter_test_dist_time <- function(df_polluter, polluter_lon, polluter_lat, polluter_projected_dist_km, upstream_threshold_dist_km, downstream_threshold_lower_dist_km, downstream_threshold_upper_dist_km, date_start, date_end, spill_date, file_path, anpoll_edgelist=NULL, projected_nodeIDs_list=NULL, flow_dist_from_list=NULL, flow_dist_to_list=NULL, flow_dist_from_list_projected=NULL, flow_dist_polluter_projected_list=NULL, df_node_latlong_anpoll=NULL, listID_nodeID_matrix=NULL, list_analyte_time=NULL){
  ...
  ## Intializing to_counter_flag that becomes 1 if we could find
  ## atleast one node within the specified threshold flow distance
  to_counter_flag<-0
  if(length(to_edgelist_ids_total)>0){
    ...
    ## Computing the flow distance indicator vector to indicate which
    ## of these nodeIDs are within the specified threshold flow
    ## distance
      flow_dist_indicator_vec<-

    ((flow_dist_to_list[[which(names(flow_dist_to_list)==polluter_node_ID)]])<=(rep(
          downstream_threshold_upper_dist_km,
          length(flow_dist_to_list[[which(names(flow_dist_to_list)==polluter_node_ID)]]))))&
    ((flow_dist_to_list[[which(names(flow_dist_to_list)==polluter_node_ID)]])>(rep(
        downstream_threshold_lower_dist_km,
        length(flow_dist_to_list[[which(names(flow_dist_to_list)==polluter_node_ID)]]))))
    ## Indexes within the "to_nodeIDs_total" vector which correspond
    ## to those nodeIDs that are within the specified threshold flow
    ## distance
    flow_dist_within_ids<-which(flow_dist_indicator_vec)
    ## Extracting the to_nodeIDs corresponding to the nodes within the
    ## specified threshold flow distance and to_edgelist_ids
    ## corresponding to row IDs in the "anpoll_edgelist" that are
    ## within specified threshold flow distance. Also updating
    ## "to_counter_flag"
    if(length(flow_dist_within_ids)>0){
  ...
  if(from_count>=1){
    for (i in 1:length(from_nodeIDs)){
      ...
      if(analyte_indicator){
        ...
        row_ids_date_interval_total<-which(dplyr::between((list_analyte_time[[listID_i]]$date), date_start, date_end))
        ...
        row_ids_date_interval_before_spill<-which(dplyr::between((list_analyte_time[[listID_i]]$date), date_start, spill_date))
        ...
        row_ids_date_interval_after_spill<-which(dplyr::between((list_analyte_time[[listID_i]]$date), spill_date, spill_date_after1year))
  ...
  if(to_counter_flag==1){
    for (i in 1:length(to_nodeIDs)){
      ...
      if(analyte_indicator){
        ...
        spill_date_POSIX$year<-spill_date_POSIX$year+1
        spill_date_after1year<-as.Date(spill_date_POSIX)
        row_ids_date_interval_total<-which((dplyr::between((list_analyte_time[[listID_i]]$date), date_start, date_end))&(dplyr::between((list_analyte_time[[listID_i]]$date), spill_date, spill_date_after1year)))
        if(length(row_ids_date_interval_total)>0){
  ...
  if(c("analyte") %in% (df_node_latlong_anpoll[which(df_node_latlong_anpoll$nodeID == polluter_node_ID), "anpoll_indicator"])){
    ...
    spill_date_POSIX$year <- spill_date_POSIX$year+1
    spill_date_after1year <- as.Date(spill_date_POSIX)
    row_ids_date_interval_total <- which((dplyr::between((list_analyte_time[[listID]]$date), date_start, date_end)) & ((dplyr::between((list_analyte_time[[listID]]$date), date_start, spill_date_after1year))))
    if(length(row_ids_date_interval_total) > 0){
      to_obs_total <- c(to_obs_total,list_analyte_time[[listID]]$conc[row_ids_date_interval_total])
      #polluter_use_flag<-1
    }
    ## Getting all downstream observations before the spill date
    row_ids_date_interval_before_spill <- which(dplyr::between((list_analyte_time[[listID]]$date), date_start, spill_date))
    if(length(row_ids_date_interval_before_spill) > 0){
    ...
    spill_date_POSIX$year <- spill_date_POSIX$year + 1
    spill_date_after1year <- as.Date(spill_date_POSIX)
    row_ids_date_interval_after_spill <- which(dplyr::between((list_analyte_time[[listID]]$date), spill_date, spill_date_after1year))
    if(length(row_ids_date_interval_after_spill) > 0){
  ...
  ########################################################################################################
  ## Third spatio-temporal test for before and after spill date
  ## upstream observations vs after spill date downstream
  ## observations.
  ...
  if((from_n_total > 0) & (to_n_after_spill > 0)){
    ...
    summary_total <- c(from_mean_total, to_mean_total,
                       from_median_total, to_median_total,
                       from_n_total, to_n_total,
                       p_value_t.test_1sided_total,
                       p_value_wilcox.test_1sided_total,
                       test_pass_total)
    
    return(list("summary_upstream" = summary_upstream,
                "summary_downstream" = summary_downstream,
                "summary_total" = summary_total,
                "All_upstream_obs" = from_obs_total,
                "All_downstream_obs" = to_obs_total,
                "After_spill_downstream_obs" = to_obs_after_spill))
}
...
fdr_analysis_wrapper <- function(polluter_test_matrix, alpha, county, file_path, df_polluter_processed=NULL){
  ...
  names(df_polluter_test_mean) <- c("County", "Longitude_Original", "Latitude_Original", "Longitude_Mapped", "Latitude_Mapped", "Date", "Upstream Mean (ppb)","Downstream Mean (ppb)","No. of Observations Upstream","No. of Observations Downstream","t test (up) p values","t test (down) p values", "t test (updown) p values", "version_1_decision", "version_2_decision", "version_3_decision")
  ...
  names(df_polluter_test_median) <- c("County", "Longitude_Original", "Latitude_Original", "Longitude_Mapped", "Latitude_Mapped", "Date", "Upstream Median (ppb)","Downstream Median (ppb)","No. of Observations Upstream","No. of Observations Downstream","Wilcox (up) p values","Wilcox (down) p values","Wilcox (updown) p values", "version_1_decision", "version_2_decision", "version_3_decision")
...
## Defining a wrapper function to calculate flow distance of all polluter nodes in parallel
wrapper_flow_dist_cal <- function(index, df_polluter,anpoll_edgelist, shortest_path_anpoll_edgelist, total_edgelist_character_modified, from_indicator, to_indicator, flow_type, file_path){
...
wrapper_polluter_test <- function(index, n_chunks, file_path, j, df_threshold_dist_km, df_polluter_processed=NULL){
    ## Loading required libraries
    library(geosphere)
    library(network)
    library(igraph)
  ...
  wrapper_polluter_test_update <- function(index, n_chunks, df_polluter_to_append, file_path, j, df_threshold_dist_km, df_polluter_processed_to_append, df_polluter_processed=NUULL){
    ## Loading required libraries
    library(geosphere)
    library(network)
    library(igraph)
